...
OS <- fread("../../OS/OS.csv",na.string="") %>% filter(before_2018==1)
...
vars=c("CANCER_TYPE","START_YEAR","GENDER","PROFILE_AFTER_TREATMENT","TREATMENT_CUM","LINE_OF_TREATMENT","PANEL_VERSION","AGE_AT_TREATMENTSTART","ANCESTRY_ROT_PC1","ANCESTRY_ROT_PC2","ANCESTRY_EA_PC1","ANCESTRY_EA_PC2")
...
temp <- merge(OS_C,fread("../adverse_events_truenegcorrected.csv",na.strings="") %>% select(MRN,ADV_EVENT,ADV_EVENT_DT),by=c("MRN")) %>% mutate(irAE_time=as.integer(ifelse(ADV_EVENT,as.integer(difftime(ADV_EVENT_DT,IO_START,unit="days")),tstop)),irAE=as.numeric(ifelse(ADV_EVENT,1,0))) %>% select(-ADV_EVENT_DT,-ADV_EVENT)
...
fwrite(result,paste("result_coxmeg_truenegcorrected/",chr_number,"_part",part,sep=""))
