...
chr_number = args[[1]]
part =formatC(as.integer(args[[2]])-1, width = 2, format = "d", flag = "0")
fold = args[[3]]
...
fold_samples <- read.table(paste("crossval_folds/",fold,"_train",sep=""))$V1
sample_names <- intersect(sample_names,fold_samples)
...
temp <- merge(OS_C,fread("../adverse_events_predicted.csv",na.strings="") %>% select(MRN,ADV_EVENT,ADV_EVENT_DT),by=c("MRN")) %>% mutate(irAE_time=as.integer(ifelse(ADV_EVENT,as.integer(difftime(ADV_EVENT_DT,IO_START,unit="days")),tstop)),irAE=as.numeric(ifelse(ADV_EVENT,1,0))) %>% select(-ADV_EVENT_DT,-ADV_EVENT)
...
colnames(kinship_raw) <- c(t(suppressWarnings(read.table("/data/gusev/PROFILE/GWAS_Surv/IO_COHORT/KINSHIP/kinship.matrix.rel.id"))))
...
fwrite(result,paste("result_coxmeg_crossval/",fold,"/",chr_number,"_part",part,sep=""))
